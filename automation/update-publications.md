...
verified ones to `docs/data/publications.yaml`, and prepare the resulting changes
for human review.

This is the canonical procedure for the recurring publications update. Agent-
specific wrappers may control how the resulting branch or pull request is
published, but they must not weaken the discovery, verification, filtering, or
validation rules in this file.
...
```bash
curl -s "https://api.openalex.org/works?filter=cites:${OPENALEX_ID},from_publication_date:${FROM}&per-page=200&cursor=*&select=id,doi,title,publication_year,publication_date,authorships,primary_location,type,is_retracted,cited_by_count&mailto=nsheff@databio.org" \
...
(OpenAlex `cited_by_count`) and note the truncation in the PR summary. The next
...
Ensure PyYAML is available, then run:

```bash
python validate_publications.py --check-dois
```

If it fails, fix the data and re-run. **Do not publish changes when the
validator fails.**

### 12. Prepare the review summary

If changes are being published as a pull request, its description must contain:
...
That rejection list is what makes the update reviewable. It is how a human
catches a filter that has become too aggressive. Do not omit it.

Never merge the resulting pull request automatically. A human reviews every
publication update.

### 13. If nothing new is found

Make no repository changes and stop. Do not create an empty branch, commit, or
pull request.
