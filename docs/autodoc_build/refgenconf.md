...
```python
def get_asset(self, genome_name, asset_name, tag_name=None, seek_key=None, strict_exists=True, check_exist=<function RefGenConf.<lambda> at 0x7feef90d0290>, enclosing_dir=False)
```
...
```python
def list_remote(self, genome=None, order=None, get_url=<function RefGenConf.<lambda> at 0x7feef90d0680>)
```
...
```python
def pull_asset(self, genome, asset, tag, unpack=True, force=None, get_json_url=<function RefGenConf.<lambda> at 0x7feef90d08c0>, build_signal_handler=<function _handle_sigint at 0x7feef9187680>)
```
...
*Version Information: `refgenconf` v0.6.2, generated by `lucidoc` v0.4.2*