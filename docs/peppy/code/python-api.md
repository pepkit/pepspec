...
- `pep_dictionary` (`dict`):  in-memory dict representation of pep.




```python
def from_pandas(self, samples_df: pandas.core.frame.DataFrame, sub_samples_df: List[pandas.core.frame.DataFrame]=None, config: dict=None) -> 'Project'
```
...
- `expand` (`bool`):  whether to expand the paths
...
*Version Information: `peppy` v0.40.0a4, generated by `lucidoc` v0.4.3*