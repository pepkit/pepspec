...
First, we will need to add a `project_interface` to a `pipeline_interface`. We can simply modify the previous one:
...
- once we've plotted the results in a graph, we can report the location of the saved graph using pipestat. Note that the structure of the reported image, `regions_graph` is a nested dictionary and contains all the required fields from our modified pipestat output schema.
